...
Parses HTML strings into Node trees.
...
def parse_html(html_string: str) -> Node:
    ...
    # Convert lxml tree to Node tree
...
def _convert_lxml_to_node(lxml_node) -> Node:
    """
    Recursively convert lxml Element to Node.
    ...
    # Create Node
    node = Node(tag=tag_name, attrib=attrib, styles=styles, bounds=bounds, metadata=metadata)
